...
Concretely, I computed the covariance matrix of responses to various survey questions about views on policy and ideology and took my measure to be the matrix's spectral radius.
This captures both the degree to which people disagree on individual issues \emph{and} the between-issue correlation of opinions (\cref{sec:theory}).

Next, I presented two decompositions that explored not just whether polarization has increased, but \emph{why}.
...
For example, it asks no long-running questions about immigration.
...
Does highly-polarized discussion on Twitter (in the affective sense) correlate with the ideological polarization of the broader public?
...
Producing such fine estimates would require reckoning with finite-sample bias in the naïve spectral radius estimator for extremely small cells, among other statistical issues.%
\footnote{
	Putting a prior on the covariance matrix (or its diagonalizing operator) would reduce this bias at the cost of imposing an assumption that people disagree similarly (or on similar issues) across the whole country.
...
Altogether, I hope that this work provides a meaningful contribution to the discourse around political polarization and serves as a useful foundation for future analysis.
