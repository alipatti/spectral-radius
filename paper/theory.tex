...
\subsection{Statistical Formalization}
\label{sec:formalization}

We now we formalize our data setup and estimand, present an estimator, and prove some basic properties about it.
%
In particular, we assume that we observe $\mathbf x_1, \ldots, \mathbf x_p \sim \mathcal F$ with $\mathbf x_i = \paren{x_{i1}, \ldots, x_{ip}}$.
...
The estimand of interest is $\rho = \norm{\Sigma}_2$, the spectral radius of $\Sigma$.

My estimator is quite straightforward:
I define the sample covariance matrix $\widehat S_n \ceq \frac 1n \sum_i \mathbf x_i \mathbf x_i \trans$ and take the naive estimator $\hat \rho = \| \widehat S_n \|_2$.
...
My setup is intentionally model-free, however, it does play nicely with a one-dimensional latent model.
In particular, the spectral radius (eventually) picks up increases in spread within the latent distribution.
...
\begin{proposition}
	...
	\begin{equation}
		x_{ij} = \beta_j y + e_i;
		\hspace{1in}
		\Var(\mathbf e) = \Gamma \in \R^{d \times d}.
	\end{equation}
	Define $\Sigma = \Var(\mathbf x_i)$ and $r = \norm{\Sigma}_2$.
	Then,
	\begin{enumerate}
		\item $r$ is non-decreasing with respect to $a$ and
		\item if $\beta \neq 0$, there exists some $A > 0$ such that for $a > A$, $r$ is strictly increasing with respect to $a$.
	\end{enumerate}
\end{proposition}
